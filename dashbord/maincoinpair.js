...
const TechnicalAnalysisComponent = (function () {
  ...
  // Globals for custom S/R selection
  ...
  const actionHTML = `
    <div class="bg-gray-50 p-6 rounded-lg border border-gray-200">
      ...
      <div class="grid md:grid-cols-2 gap-4 mb-6">
        <!-- Action Card -->
        <div class="bg-white p-4 rounded-lg border ${rec.action === 'BUY' || rec.action === 'STRONG_BUY'
          ? 'border-green-200' : 'border-red-200'}">
          <div class="flex items-center gap-3 mb-2">
            <span class="p-2 rounded-lg ${rec.action === 'BUY' || rec.action === 'STRONG_BUY'
              ? 'bg-green-100' : 'bg-red-100'}">
              ${rec.action === 'BUY' || rec.action === 'STRONG_BUY' ? '🔼' : '🔽'}
            </span>
            <div>
              <p class="text-sm text-gray-600">Recommended Action</p>
              <p class="text-lg font-bold ${rec.action === 'BUY' || rec.action === 'STRONG_BUY'
                ? 'text-green-600' : 'text-red-600'}">${rec.action}</p>
            </div>
          </div>
        </div>
